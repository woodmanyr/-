...
if Request.Form("submit")="确 定" then
 ...
 for i=1 to Request.Form("hidden").count
   strSel="Select id,flag from student where id="&Request.Form("hidden")(i)
 ...
 for i=1 to Request.Form("hidden").count
   strSel="Select * from student where id="&Request.Form("hidden")(i)
   rs_good.Open strSel,conn,3,2
            ...
            <td width="100%" height="14"> <div align="center"> 
                <table border="1" cellpadding="0" cellspacing="0" width="90%" bordercolorlight="#0A5092" bordercolordark="#FFFFFF" height="122">
                  <tr> 
                    ...
                    <td width="50%" bgcolor="#FFFFFF"><div align="right"><a href="student_hangban_add.asp?page=<%=Request("page")%>">添加航班&gt;&gt;&gt;</a></div></td>
                  </tr>
                  <tr> 
                    <td colspan="2"><center> 
                  <tr> 
              ...
              if rs.Eof then exit for  
                %>
				  ...
				  %>
                          ...
                          <td width="12%" height="18" align="center"><a href="student_hangban_edit.asp?id=<%=rs("id")%>&page=<%=request("page")%>"><font color="#000000">[修改]</font></a><font color="#000000">&nbsp; 
                            </font><a href="student_hangban_del.asp?id=<%=rs("id")%>&page=<%=request("page")%>"><font color="#000000">[删除]</font></a></td>
                        </tr>
                        <%
                rs.movenext
                If rs.eof then exit for
                  ...
                  <tr> 
                    <td width="100%" height="26" colspan="2" valign="middle"> 
                      <p align="center"><font color="#333333"><b>共 <font color="#990000"><%=rs.pagecount%> 
                        </font> 页 </b>&nbsp;<font face="webdings"><a href="student_hangban.asp?page=1"><font color="#484848">9</font></a></font> 
                        <font face="webdings"><a href="student_hangban.asp?page=<%=page-1%>"><font color="#484848">7</font></a></font>[ 
                        <%
      for i=1 to rs.pagecount
      if i=page then
      response.write "<font color=""#990000""><b>"&page&"&nbsp;</b></font>"
      else
      response.write "<A HREF=student_hangban.asp?page="&i&">"&"<font color=#000000>"&i&"</font>"&"</A>&nbsp;"
      end if
      next
      
     
            %>
                        ]<font face="webdings"><a href="student_hangban.asp?page=<%=page+1%>"><font color="#484848">8</font></a></font> 
                        <font face="webdings"><a href="student_hangban.asp?page=<%=rs.PageCount%>"><font color="#484848">:</font></a> 
